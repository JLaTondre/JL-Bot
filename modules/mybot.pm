...
sub getNamespaces {
    ...
    for my $number ( keys %{ $results->{query}->{namespaces} } ) {
        ...
        my $canonical = $results->{query}->{namespaces}->{$number}->{'*'};
...
sub getText {
    ...
    # This subroutine gets the text, timestamp, & revision of a page.  It is passed the
    # page to get.
    ...
    return $text, $timestamp, $revision;
}
...
sub purgePage {
    ...
    unless ($page) {
        croak "\nMyBot->purgePage: Page not specified!";
    }
...
sub getPageLinks {
    ...
    unless ($page) {
        croak "\nMyBot->getPageLinks: Page not specified!";
    }
...
sub getRedirects {
    ...
    my $results = _api($self, "MyBot->getRedirects", {
        action      => 'query',
        prop        => 'redirects',
        titles      => $page,
        rdnamespace => $namespaces,
        rdprop      => 'title',
        rdlimit     => 'max'
    } );
    ...
    my $redirects;    # hard ref

    for my $redirect ( @{ $data->{redirects} } ) {
        $redirects->{ $redirect->{title} } = 1;
    }
...
sub getDefaultSort {
    ...
    my $id = ( keys %{ $results->{query}->{pages} } )[0];

    if (exists $results->{query}->{pages}->{$id}->{pageprops}->{defaultsort}) {
        return $results->{query}->{pages}->{$id}->{pageprops}->{defaultsort};
    }

    # defaultsort not set so use page name
